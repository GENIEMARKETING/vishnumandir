  ...
  <tr><td align="center">
    <table width="600" cellpadding="0" cellspacing="0" style="background:#fff;border-radius:16px;overflow:hidden;box-shadow:0 4px 20px rgba(0,0,0,.08)">
      <tr><td style="background:#8B2E0F;padding:32px 40px;text-align:center">
        ...
        <p style="margin:4px 0 0;color:#e8c99a;font-size:13px">New Facility Request Received</p>
      </td></tr>
      <tr><td style="background:#C5A059;height:3px"></td></tr>
      <tr><td style="padding:40px">
        <h2 style="color:#8B2E0F;font-size:22px;margin:0 0 20px;font-weight:normal">📋 New Facility Request</h2>
        <p style="color:#374151;line-height:1.7;margin:0 0 20px">A new facility request has been submitted and is awaiting your review.</p>
        <table width="100%" cellpadding="0" cellspacing="0" style="border-top:1px solid #f0ece5;padding-top:20px;margin-top:4px">
          <tr><td style="padding:8px 0;color:#6b7280;font-size:14px;width:160px">Name</td><td style="padding:8px 0;color:#1f2937;font-size:14px;font-weight:bold">${result.requesterName || ""}</td></tr>
          <tr><td style="padding:8px 0;color:#6b7280;font-size:14px">Email</td><td style="padding:8px 0;color:#1f2937;font-size:14px;font-weight:bold">${result.requesterEmail || ""}</td></tr>
          <tr><td style="padding:8px 0;color:#6b7280;font-size:14px">Phone</td><td style="padding:8px 0;color:#1f2937;font-size:14px;font-weight:bold">${result.requesterPhone || ""}</td></tr>
          <tr><td style="padding:8px 0;color:#6b7280;font-size:14px">Event Type</td><td style="padding:8px 0;color:#1f2937;font-size:14px;font-weight:bold">${result.eventType || ""}</td></tr>
          <tr><td style="padding:8px 0;color:#6b7280;font-size:14px">Event Name</td><td style="padding:8px 0;color:#1f2937;font-size:14px;font-weight:bold">${result.eventName || "—"}</td></tr>
          ...
          <tr><td style="padding:8px 0;color:#6b7280;font-size:14px">Start / End</td><td style="padding:8px 0;color:#1f2937;font-size:14px;font-weight:bold">${result.startTime || "—"} – ${result.endTime || "—"}</td></tr>
          <tr><td style="padding:8px 0;color:#6b7280;font-size:14px">Guests</td><td style="padding:8px 0;color:#1f2937;font-size:14px;font-weight:bold">${result.numberOfGuests || ""}</td></tr>
          <tr><td style="padding:8px 0;color:#6b7280;font-size:14px">Details</td><td style="padding:8px 0;color:#1f2937;font-size:14px">${result.details || "—"}</td></tr>
          <tr><td style="padding:8px 0;color:#6b7280;font-size:14px">Requirements</td><td style="padding:8px 0;color:#1f2937;font-size:14px">${result.requirements || "—"}</td></tr>
      ...
      await sendResendEmail(adminEmail, `New Facility Request: ${result.eventType || "Request"} — ${result.requesterName || ""}`, html);
    } catch (err) {
      strapi.log.error("[facility-request lifecycle] afterCreate admin notify error:", err);
    }
  },
